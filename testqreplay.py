...
def weight_variable(shape):
  initial = tf.truncated_normal(shape, stddev=1.5e-2)
...
W_fc1 = weight_variable([21 * 16 * features2, aggregate])
b_fc1 = bias_variable([aggregate])
keep_prob = tf.placeholder(tf.float32)
h_pool2_flat = tf.reshape(h_pool2, [-1, 21*16*features2])
...
trainer = tf.train.RMSPropOptimizer(learning_rate=1e-5,decay=.99)
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while True:
            ...
            allQ = sess.run(Qout,feed_dict={inputs1:s1_mat-s_mat, keep_prob: 1})
            a=[0]
