...
inputs1 = tf.placeholder(shape=[None,160,130],dtype=tf.float32)


def preprocess(image):
  avg = np.matrix([[0.33],[0.33],[0.33]],dtype=np.float32)   
  image = np.dot(image,avg)*(1/255)
  image = image[35:195,15:145]
  return image
...
x_image = tf.reshape(inputs1, [-1,160,130,1])
h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)
h_pool1 = max_pool_2x2(h_conv1)
h_pool1_flat = tf.reshape(h_pool1, [-1, 5200*features1])

W_fc1 = weight_variable([5200*features1, aggregate])
...
one_hot = tf.placeholder(shape=[None,3,1],dtype=tf.float32)
loss = tf.batch_matmul(tf.square(nextQ - y_conv),one_hot)
    ...
    for i in range(num_episodes):
        ...
        while True:
            ...
            if (rand < epsilon):
              ...
              if epsilon >.01 and len(good_memories)*len(bad_memories)>0:
            ...
            if not r==0 or d==True:
                ...
                if(len(bad_memories)>batch_size):
                  ...
                  for inst in sample:
                    ...
                    one = [[0],[0],[0]]
                    one[inst[4][0]] = [1]
                    total_input.append(inst[0])
                    total_target.append(targetQ)
                    total_onehot.append(one)
                if(len(good_memories)>batch_size):
                  ...
                  for inst in sample:
                    ...
                    total_target.append(targetQ)
                    total_onehot.append(one)
                if(len(good_memories)>0 and len(bad_memories)>0):
                  sess.run([updateModel],feed_dict={inputs1:total_input,nextQ:total_target,one_hot:total_onehot,keep_prob: 0.5})
                memory = []
                if d==True:
                  print(reward_mem)
                  break
